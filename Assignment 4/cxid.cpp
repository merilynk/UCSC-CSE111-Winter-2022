...
void reply_put (accepted_socket& client_sock, cxi_header& header) {
   ...
   ofstream file_recieved (header.filename, ofstream::binary);
   if (file_recieved) {
      cout << "file opened" << endl;
      header.command = cxi_command::ACK;
      file_recieved.seekp(0);
      file_recieved.write(buffer, header.nbytes + 1);
   } else {
      cout << "file failed to open" << endl;
      header.command = cxi_command::NAK;  
      return;    
...
void run_server (accepted_socket& client_sock) {
   ...
   try {   
      for (;;) {
         ...
         // cerr << "nbytes of header: " << header.nbytes << endl;
         // cerr << "size of header: " << sizeof header << endl;
         // cerr << "in run_server, header.command: " 
         //    << command_to_string(header.command) << endl;
